...
class ManufacturerBrandLookup:
    ...
    def from_dataframe(
        ...
        lookup = build_manufacturer_brand_lookup(df)
    ...
    def resolve(
    ...
    ) -> Optional[ManufacturerBrandRecord]:
        ...
        for candidate_key, record in self.lookup.items():
...
class DecimalFractionLookup:
    """
    Lookup between fraction strings and decimal values.

    The internal dictionaries use *_map names so that we can also expose
    callable methods named fraction_to_decimal() and decimal_to_fraction().
    """

    fraction_to_decimal_map: Dict[str, float] = field(default_factory=dict)
    decimal_to_fraction_map: Dict[float, str] = field(default_factory=dict)
    ...
    def from_dataframe(
        ...
        """
        Build a DecimalFractionLookup from a dataframe.

        Supports both:
            fraction / decimal

        and wide formats such as:
            Fraction_1 / Decimal_1
            Fraction_2 / Decimal_2
            ...
        """
        ...
        # ---------------------------------------------------------------
        if "fraction" in df.columns and "decimal" in df.columns:
            for _, row in df.iterrows():
                ...
                result.fraction_to_decimal_map[frac] = dec
                result.decimal_to_fraction_map[dec] = frac
        ...
        fraction_columns = [
            col
            for col in df.columns
            if str(col).strip().lower().startswith("fraction")
        ]

        for fraction_col in fraction_columns:
            fraction_col_str = str(fraction_col).strip()

            # Example:
            # Fraction_1 -> 1
            # Fraction_2 -> 2
            parts = fraction_col_str.split("_", 1)

            if len(parts) == 2:
                suffix = parts[1]
                decimal_col = f"Decimal_{suffix}"
            else:
                decimal_col = "Decimal"

            # Case-insensitive fallback
            if decimal_col not in df.columns:
                matching_decimal_col = None

                for col in df.columns:
                    if str(col).strip().lower() == decimal_col.lower():
                        matching_decimal_col = col
                        break

                if matching_decimal_col is None:
                    continue

                decimal_col = matching_decimal_col

            for _, row in df.iterrows():
                frac = clean_value(row.get(fraction_col))
                dec = row.get(decimal_col)
    ...
    def fraction_to_decimal(
        ...
        Returns None if the fraction is not present.
        """

        if is_placeholder(fraction):
            return None

        return self.fraction_to_decimal_map.get(
            str(fraction).strip()
        )
    ...
    def decimal_to_fraction(
        ...
        """
        Convert a decimal to the nearest known fraction.

        Example:
            0.5 -> "1/2"

        Returns None if no known fraction is within tolerance.
        """

        best_fraction = None
        best_diff = float("inf")

        for decimal, fraction in self.decimal_to_fraction_map.items():
        ...
        if (
            best_fraction is not None
            and best_diff <= tolerance
        ):
            return best_fraction

        return None

    def decimal_to_fraction_str(
        self,
        value: float,
        tolerance: float = 1e-4,
    ) -> Optional[str]:
        """
        Backward-compatible wrapper for decimal_to_fraction().
        """

        return self.decimal_to_fraction(
            value,
            tolerance,
        )

    def mixed_number_to_fraction_str(
        self,
        value: float,
    ) -> Optional[str]:
        """
        ...
        frac_str = self.decimal_to_fraction(
            frac_part
        )
...
def build_decimal_fraction_lookup(
    ...
    """
    Build the decimal/fraction lookup from a dataframe.
    """

    return DecimalFractionLookup.from_dataframe(df)
