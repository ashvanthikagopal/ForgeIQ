...
def normalize_manufacturer_brand(
    ...
    if lookup is None:
        has_mfg_or_brand = bool(raw_manufacturer or raw_brand)
        return ManufacturerBrandResult(
            manufacturer=raw_manufacturer,
            brand=raw_brand,
            matched=has_mfg_or_brand,
            needs_review=not has_mfg_or_brand,
            review_reason=None if has_mfg_or_brand else "Manufacturer/Brand is missing.",
        )
